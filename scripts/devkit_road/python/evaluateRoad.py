...
def main(result_dir, train_dir, debug = False):
    ...
    for cat in dataStructure.cats:
        ...
        submission_tag = dataset_tag + '_'
        print "Searching for submitted files with prefix: %s" %submission_tag
        
        for i in range(0,len(gt_fileList),2):
            if(cat=='uu_road'):
                fn_curGt = gt_fileList[i+1]
            else:
                fn_curGt = gt_fileList[i]
            ...
            img = cv2.imread(fn_curProb,cv2.COLOR_BGR2GRAY)
            cur_prob = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            cv2.imshow('img',cur_prob)
...
if __name__ == "__main__":
    ...
    # parse parameters
    result_dir = 'C:\\Users\\yangf\\Desktop\\Workspace\\ALE\\Result\\KITTI\\val\\Crf_HFM'
    gt_dir = 'D:\\Data\\KITTI\\data_road\\training'
