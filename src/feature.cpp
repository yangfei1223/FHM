...
void LTextonFeature::Train(LList<char *> &trainImageFiles)
{
	...
	//�����˲����
	filterData = new double[totalSize * filterCount];		//filterCount���Ѿ�����ͨ����
	dataTo = filterData;
	...
	for (i = 0; i < trainImageFiles.GetCount(); i++)
	{
		...
		for (j = 0; j < filters.GetCount(); offset += filters[j]->GetBands(), j++)
			filters[j]->Filter(labImage, 0, filterResponse, offset, subSample);		//�˲�
...
int LTextonFeature::GetTotalFilterCount()
{
	int filterCount = 0;
	for (int i = 0; i < filters.GetCount(); i++)
		filterCount += filters[i]->GetBands();		//ÿ��ͨ����Ҫ�����˲�
	return(filterCount);
}
...
void LTextonFeature::Discretize(LLabImage &labImage, LImage<unsigned short> &image, const char *imageFileName)
{
	...
	int offset = 0;
	for (j = 0; j < filters.GetCount(); offset += filters[j]->GetBands(), j++)
		filters[j]->Filter(labImage, 0, filterResponse, offset, 1);		//�˲�

	image.SetResolution(width, height, 1);		//���õ�ͨ��
	for (i = 0; i < height; i++)
		for (j = 0; j < width; j++)
			image(j, i, 0) = clustering->NearestNeighbour(filterResponse(j, i));	//���䵽����ľ�������
}
...
LLidarFeature::LLidarFeature(LDataset * setDataset, const char * setTrainFolder, const char * setTrainFile, const char * setEvalFolder, const char * setEvalExtension, int setNumberOfClusters, double setKMeansMaxChange, int setPointsPerKDTreeCluster, int setFeatureCount) : LDenseFeature(setDataset, setTrainFolder, setTrainFile, setEvalFolder, setEvalExtension)
{
	numberOfClusters = setNumberOfClusters, kMeansMaxChange = setKMeansMaxChange, pointsPerKDTreeCluster = setPointsPerKDTreeCluster;
	featureCount = setFeatureCount;
	clustering = new LKMeansClustering<double>(setTrainFolder, setTrainFile, featureCount, numberOfClusters, kMeansMaxChange, pointsPerKDTreeCluster);
}


LLidarFeature::~LLidarFeature()
{
	delete clustering;
}

void LLidarFeature::Train(LList<char*>& trainImageFiles)
{
	int x, y;
	int fileSum = trainImageFiles.GetCount(),featureCount=7,totalSize = 0, i, j;
	double *lidarData, *dataTo;

	//ÿ��ͼ��ʼ�˲�
	for (i = 0; i < trainImageFiles.GetCount(); i++)
	{
		char *fileName, *calibfileName, *lidarfileName;
		//��ȡ�ļ���
		fileName = GetFileName(dataset->imageFolder, trainImageFiles[i], dataset->imageExtension);
		//����ͼ��
		LRgbImage rgbImage(fileName);
		...
		lidarfileName = GetFileName(dataset->lidarFolder, trainImageFiles[i], dataset->lidarExtension);
		calibfileName = GetFileName(dataset->calibFolder, trainImageFiles[i], dataset->calibExtension);

		//������
		m_Lidar.DoNext(fileName, calibfileName, lidarfileName);		//�����㷨
		delete[] lidarfileName;
		delete[] calibfileName;
		for (y=0;y<rgbImage.GetHeight();y++)
		{
			for (x=0;x<rgbImage.GetWidth();x++)
			{
				if (m_Lidar.m_Valid2Img.at<uchar>(y,x)>0)
				{
					totalSize++;
				}
			}
	...
	//�����˲����
	lidarData = new double[totalSize * featureCount];		//filterCount���Ѿ�����ͨ����
	dataTo = lidarData;

	printf("Calculate lidar features...\n");

	//��ÿ��ͼ������˲�
	for (i = 0; i < trainImageFiles.GetCount(); i++)
	{
		printf("extract image %d %s..\n", i, trainImageFiles[i]);
		char *fileName,*gtfileName, *calibfileName, *lidarfileName;
		fileName = GetFileName(dataset->imageFolder, trainImageFiles[i], dataset->imageExtension);
		LLabImage labImage(fileName);
		delete[] fileName;

		gtfileName = GetFileName(dataset->groundTruthFolder, trainImageFiles[i], dataset->groundTruthExtension);
		lidarfileName = GetFileName(dataset->lidarFolder, trainImageFiles[i], dataset->lidarExtension);
		calibfileName = GetFileName(dataset->calibFolder, trainImageFiles[i], dataset->calibExtension);

		//������
		m_Lidar.DoNext(fileName,gtfileName, calibfileName, lidarfileName);		//�����㷨
		delete[] gtfileName;
		delete[] lidarfileName;
		delete[] calibfileName;
		for (y = 0;y < labImage.GetHeight();y++)
		{
			for (x = 0;x < labImage.GetWidth();x++)
			{
				if (m_Lidar.m_Valid2Img.at<uchar>(y, x) > 0)
				{
					dataTo[0] = m_Lidar.m_Lidar2Img.at<Vec4f>(y, x)[0];
					dataTo[1] = m_Lidar.m_Lidar2Img.at<Vec4f>(y, x)[1];
					dataTo[2] = m_Lidar.m_Lidar2Img.at<Vec4f>(y, x)[2];
					dataTo[3] = m_Lidar.m_Lidar2Img.at<Vec4f>(y, x)[3];
					dataTo[4] = m_Lidar.m_Curvature.at<float>(y, x);
					dataTo[5] = m_Lidar.m_MaxHeightDiff.at<float>(y, x);
					dataTo[6] = m_Lidar.m_HeightVar.at<float>(y, x);
					dataTo += 7;
				}
			}
		}
	...
	//�������������о���
	clustering->Cluster(lidarData, totalSize);
	delete[] lidarData;
...
void LLidarFeature::Discretize(LLabImage & labImage, LImage<unsigned short>& image, const char * imageFileName)
{
}

void LLidarFeature::LoadTraining()
{
}

void LLidarFeature::SaveTraining()
{
}

int LLidarFeature::GetBuckets()
{
	return 0;
}
