...
void LMeanShiftFilter2D<T>::Filter(LImage<T> &imageFrom, int bandFrom, LImage<T> &imageTo, int bandTo, int sampleSize)
{
	...
	for (j = 0; j < height; j++)
		for (i = 0; i < width; i++, xyData += imageBands + 2, fromData += imageBands)	//��ָ������
		{
			//������ͨ���Ǵ��i * invXY��j * invXY�ģ����ռ���Ϣ
			xyData[0] = (T)(i * invXY), xyData[1] = (T)(j * invXY);
			for (k = 0; k < imageBands; k++)		//��ͨ��
				xyData[k + 2] = (T)(fromData[k] * invLuv);	//ʣ�µ�ͨ�������ɫ��Ϣ
		}

	//�����С��ֵ
	double rangeMins[3], rangeMaxs[3];
	xyData = &xyImage(0, 0, 2);	//����ָ��ָ�����ݵ�Lͨ��
	...
	{
		if (*xyData < rangeMins[2])
			rangeMins[2] = *xyData;
		else
			if (*xyData > rangeMaxs[2])
				rangeMaxs[2] = *xyData;
	}

	//����б�27������noBuckets��
	int *buckets, *bucketList, bucketNeighs[27], noBuckets[3];		//27���������İ�

	for (i = 0; i < 3; i++)
		noBuckets[i] = (int)(rangeMaxs[i] - rangeMins[i] + 3);		//�����
	buckets = new int[noBuckets[0] * noBuckets[1] * noBuckets[2]];		//�������
	for (i = 0; i < noBuckets[0] * noBuckets[1] * noBuckets[2]; i++)
		buckets[i] = -1;		//��ʼ��Ϊ-1
	...
	{
		xyData = xyImage(0, l * sampleSize);	//��ָ��
		for (i = 0; i < subWidth; i++, xyData += (imageBands + 2) * sampleSize)
			if (modeTable[i + l * subWidth] != 1)
			{
					...
					if (!skipNeighbours)
					{
						...
						if ((modeTable[modeCandidate] != 2) && (modeCandidate != i))
						{
							...
							if (diff < 0.5)
							{
								...
								{
									for (j = 0; j < imageBands; j++)
										point[j + 2] = modeData[j] / sigmaLuv;
									modeTable[i] = 1;
									meanAbs = -1;
								}
							}
				...
				if (!skipNeighbours)
				{
					for (j = 0; j < pointCount; j++)
					{
						...
						for (k = 0; k < imageBands; k++)
							modeData[k] = (T)point[k + 2];
					}
				}
				T *modeData = &imageTo(i, l, bandTo);
				for (j = 0; j < imageBands; j++)
					modeData[j] = (T)point[j + 2];
			}
	}
	//�ͷ��ڴ�
	delete[] modeTable;
