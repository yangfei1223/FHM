...
void LImage<T>::WindowTo(LImage &imageTo, int centreX, int centreY, int halfWidth, int halfHeight)
{
	...
	{
		int posY = centreY - halfHeight + i;	//y轴
		//lineFrom指向当前行，保证在图像返回内
		T *lineFrom = (*this)(0, (posY < 0) ? 0 : ((posY > height - 1) ? height - 1 : posY));
		//要拷贝到的窗口的行
		T *lineTo = imageTo(0, i);
		//列j
		for (j = 0; (j < halfWidth - centreX) && (j < 2 * halfWidth + 1); j++) 
			memcpy(lineTo + j * bands, lineFrom, bands * sizeof(T));
		for (j = (width - centreX + halfWidth >= 0) ? width - centreX + halfWidth : 0; j < 2 * halfWidth + 1; j++) 
			memcpy(lineTo + j * bands, lineFrom + (width - 1) * bands, bands * sizeof(T));
		int diff = ((halfWidth - centreX > 0) ? halfWidth - centreX : 0) + ((halfWidth + 1 > width - centreX) ? halfWidth + 1 - width + centreX : 0);
		if (2 * halfWidth + 1 > diff) 
			memcpy(lineTo + ((halfWidth - centreX > 0) ? halfWidth - centreX : 0) * bands, lineFrom + ((centreX - halfWidth > 0) ? centreX - halfWidth : 0) * bands, (2 * halfWidth + 1 - diff) * bands * sizeof(T));
	}
}
...
void LRgbImage::Load(char *fileName)
{
	...
	SetResolution(cvImage.cols, cvImage.rows);
	CopyDataFrom(cvImage.data);
}
...
//写图像到文件，有那么一点问题，不过不是什么大问题，数据是对的
void LRgbImage::Save(char *fileName)
{
	cv::Mat cvImage;
	cvImage.create(height, width, CV_8UC3);
	CopyDataTo(cvImage.data);
/*	cv::imshow("res", cvImage);
	cv::waitKey(0)*/;
...
{
	cv::Mat cvImage;
	cvImage.create(height, width, CV_8U);
